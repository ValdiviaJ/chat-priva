...
export const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  return (
    <header className="h-16 px-4 sm:px-6 bg-[#080d1a] border-b border-[#1a2333] flex items-center justify-between shrink-0 sticky top-0 z-30">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            {isEditingTitle ? (
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  autoFocus
                  className="bg-[#151f32] text-white text-sm font-semibold px-2 py-0.5 rounded border border-blue-500 focus:outline-none"
                />
                <button
            ...
            {connectionState === 'disconnected' ? (
              <span title="Sin conexión" className="text-rose-400 flex items-center">
                <WifiOff className="w-3.5 h-3.5" />
              </span>
            ) : connectionState === 'connecting' ? (
              <span title="Conectando..." className="text-amber-400 flex items-center">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400">
            <span
              className={`w-2 h-2 rounded-full ${
                isOtherOnline ? 'bg-emerald-400' : 'bg-slate-500'
              }`}
            />
