...
export const MessageBubble: React.FC<MessageBubbleProps> = React.memo(({ message, isMe }) => {
  ...
  return (
    ...
    >
      ...
      {!isMe && (
        <div className="w-8 h-8 rounded-full bg-[#1c2e4e] border border-blue-500/30 text-blue-400 flex items-center justify-center shrink-0 mb-1">
      ...
      <div
        className={`max-w-[85%] sm:max-w-[70%] md:max-w-[55%] rounded-2xl px-4 py-3 shadow-sm text-sm relative break-words leading-relaxed ${
          isMe
            ...
            : 'bg-[#182336] text-slate-100 rounded-bl-sm border border-[#23334d]/60 shadow-none'
        }`}
      >
      ...
      {isMe && (
        <div className="w-8 h-8 rounded-full bg-[#1a2b47] border border-blue-500/40 text-blue-300 font-semibold text-xs flex items-center justify-center shrink-0 mb-1">
          {getMyInitials()}
        </div>
      )}
    </div>
